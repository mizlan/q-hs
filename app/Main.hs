...
handleEvent (VtyEvent (Ctrl k)) | k `elem` ['n', 'p'] = do
  ...
  let n'
        | k == 'n' = min numEntries . (+ 1) <$> (st ^. entrySelectPos) <|> pure 0
        | otherwise = st ^. entrySelectPos <&> \n -> max (n - 1) 0
...
googleSource q = do
  ...
  case responseBody r of
    _ : Array xs : _ -> pure $ V.fromList [x | String x <- V.toList xs]
    _ -> pure V.empty
 where
  url = http "google.com" /: "complete" /: "search"
  params = ("client" =: ("firefox" :: Text)) <> ("q" =: q)
